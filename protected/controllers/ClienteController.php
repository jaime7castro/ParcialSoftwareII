...
class ClienteController extends Controller
{
	...
	protected function performAjaxValidation($model)
	{
        ...
        public function actionViewDCP($id){
            $modelDCP= ContactoPersona::model();         
            $detalleContactoP=$modelDCP->findAllByAttributes(array('Cliente_ci'=>$id));
            $this->render('viewDCP',array('detalleContactoP'=>$detalleContactoP,'id'=>$id));
        }
        
        public function actionUpdateDCP($id){
            $modelDCP= ContactoPersona::model()->findByPk($id);
            if(isset($_POST['ContactoPersona']))
            {
                $modelDCP->attributes=$_POST['ContactoPersona'];
                if($modelDCP->save()){
                    //mostrando mensaje FLASH
                    Yii::app()->user->setFlash('success','Contacto editado con exito');
                    $this->redirect(array('viewDCP','id'=>$modelDCP->Cliente_ci));
                }
            }       
